function [S_LCTE,S_sys,xvar,SS_LCTE,SS_sys] = parametric_senseplot_vH3_notN(Xij,...
...
% "notN": this version does NOT normalize V(in) and V(out) signals
%         to unity at a particular delay time, for non-offset signals.
%         Created to replicate Fig. 2.10 of Yee Kan Koh's thesis.
%
%
% Inputs:
...
for k = 1:length(xvar)
    ...
    [Vin_model, Vout_model, deltaR_model,ratio_model, FWHM_model] = ...
...
%     return;
    ...
    for i = 1:4
        for j=1:nl
            ...
            % Call subfunction to calculate Num based on signal type.
            Num = sense_getNum(doughnut,sigfit,by_FWHM,tdelay,xvect,...
                               deltaR_temp{i,j},ratio_temp{i,j},...
                               Vin_model,Vout_model,nVout_model,ratio_model);
...
for i = 1:4
    for j = 1:nl
        ...
        if isempty(S_LCTE{i,j}), continue;
            ...
            loglog(xvar*scale,abs(S_LCTE{i,j}),LCTEmarker{i},'Color',ColorOrder(j,:));
...
for i = 1:length(sys_consider)
    ...
    if sys_consider(i)
        loglog(xvar*scale,abs(S_dat),'-','Color',ColorOrder(i,:));
...
if ~doughnut
    switch sigfit
        case 1, ylabel(sprintf('Sensitivity:  dlog[V(in)]/dlogX'), 'FontSize',18);
        case 2, ylabel(sprintf('Sensitivity:  dlog[V(out)]/dlogX'), 'FontSize',18);
...
%% Subfunction: compute reference model signal
function [Vin_model, Vout_model, deltaR_model,ratio_model, FWHM_model] = ...
    TDTR_SENS_MODEL(doughnut, sigfit, by_FWHM, tdelay, offset, ...
                    matparams, sysparams, A_pump, Zind, intscheme, nnodes)
...
% subfunction for calculating Num based on signal type. Not normalized
% V(in) or V(out) for conventional TDTR signals.
...
if ~doughnut
    switch sigfit
        case 1
            ...
            Num=log(Vin_temp)-log(Vin_model);
        case 2
            Vout_temp = imag(temp_deltaR); 
            %norm = Vout_temp(Zind);
            %nVout_temp = Vout_temp / norm;
            Num=log(Vout_temp)-log(Vout_model);
        otherwise
