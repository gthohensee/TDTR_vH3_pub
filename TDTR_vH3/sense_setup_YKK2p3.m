...
f=0.6e6; % pump laser modulation frequency, Hz
...
tdelay_max= 100e-12; % approx. max extent of our delay stage.
...
thickness = [1 100 1 100 1e9]; % thickness in nm of each model layer
stack = {'*','Al', '/', 'SiO2','Si'}; % identifier for each model layer
...
xvar = logspace(log10(50e-9),log10(5000e-9),40); % range of xvar parameter values in LCTE (SI) units
Xij = 0;
Xij = [3,4]; % LCTE matrix index of xvar parameter.
SYS_i = 0; % Scalar. 1-6 indicates xvar is [f, r_pump, r_probe, tau_rep, w0, phi], respectively.
...
LCTE_sens_consider(3,4) = 1;
LCTE_sens_consider(1,4) = 1;
sys_consider = [0 0 0 0 0 1]; %[f, r_pump, r_probe, tau_rep, w0, phi]
